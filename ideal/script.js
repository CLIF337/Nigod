// ==================== ГАРМОШКА ====================
var acc = document.getElementsByClassName("accordion");
for (var i = 0; i < acc.length; i++) {
    acc[i].onclick = function() {
        this.classList.toggle("active");
        var panel = this.nextElementSibling;
        if (panel.style.display === "block") {
            panel.style.display = "none";
        } else {
            panel.style.display = "block";
        }
    };
}

// ==================== ЗАДАЧА 1: ДАТА И ВРЕМЯ ====================
function showTime() {
    var d = new Date();
    var days = ["Воскресенье", "Понедельник", "Вторник", "Среда", "Четверг", "Пятница", "Суббота"];
    var dayOfWeek = days[d.getDay()];

    var day = d.getDate();
    var month = d.getMonth() + 1;
    var year = d.getFullYear();

    var hours = d.getHours();
    var minutes = d.getMinutes();
    var seconds = d.getSeconds();
    
    var ampm = hours >= 12 ? 'PM' : 'AM';
    hours = hours % 12;
    hours = hours ? hours : 12;

    if (day < 10) day = "0" + day;
    if (month < 10) month = "0" + month;
    if (hours < 10) hours = "0" + hours;
    if (minutes < 10) minutes = "0" + minutes;
    if (seconds < 10) seconds = "0" + seconds;

    var firstLine = year + "-" + day + "-" + month + ", " + dayOfWeek;
    var secondLine = hours + ":" + minutes + ":" + seconds + " " + ampm;

    document.getElementById("clock").innerHTML = firstLine + "<br>" + secondLine;
}
setInterval(showTime, 1000);
showTime();

// ==================== ЗАДАЧА 2: КАЛЕНДАРЬ ====================
let currentDate = new Date();
let selectedDate = null;

const months = [
    "Январь", "Февраль", "Март", "Апрель", "Май", "Июнь",
    "Июль", "Август", "Сентябрь", "Октябрь", "Ноябрь", "Декабрь"
];
const daysOfWeek = ["ПН", "ВТ", "СР", "ЧТ", "ПТ", "СБ", "ВС"];

function renderCalendar() {
    const year = currentDate.getFullYear();
    const month = currentDate.getMonth();
    
    const monthYearDisplay = document.getElementById("current-month-year");
    if (monthYearDisplay) {
        monthYearDisplay.textContent = `${months[month]} ${year}`;
    }
    
    const calendarContainer = document.getElementById("calendar-container");
    if (!calendarContainer) return;
    
    calendarContainer.innerHTML = "";
    const table = document.createElement("table");
    table.className = "calendar-table";
    
    const thead = document.createElement("thead");
    const headerRow = document.createElement("tr");
    daysOfWeek.forEach(day => {
        const th = document.createElement("th");
        th.textContent = day;
        headerRow.appendChild(th);
    });
    thead.appendChild(headerRow);
    table.appendChild(thead);
    
    const tbody = document.createElement("tbody");
    const firstDay = new Date(year, month, 1);
    const startDayOfWeek = firstDay.getDay();
    let startOffset = startDayOfWeek === 0 ? 6 : startDayOfWeek - 1;
    
    const daysInMonth = new Date(year, month + 1, 0).getDate();
    let dayCounter = 1;
    let row = document.createElement("tr");
    
    for (let i = 0; i < startOffset; i++) {
        const td = document.createElement("td");
        td.textContent = "";
        td.className = "empty-cell";
        row.appendChild(td);
    }
    
    for (let day = 1; day <= daysInMonth; day++) {
        const td = document.createElement("td");
        td.textContent = day;
        
        const currentDay = new Date(year, month, day);
        const dayOfWeekNum = currentDay.getDay();
        if (dayOfWeekNum === 0 || dayOfWeekNum === 6) {
            td.classList.add("weekend");
        }
        
        const today = new Date();
        if (today.getFullYear() === year && today.getMonth() === month && today.getDate() === day) {
            td.classList.add("today");
        }
        
        if (selectedDate && selectedDate.getFullYear() === year && selectedDate.getMonth() === month && selectedDate.getDate() === day) {
            td.classList.add("selected");
        }
        
        td.addEventListener("click", (function(y, m, d) {
            return function() { selectDate(y, m, d); };
        })(year, month, day));
        
        row.appendChild(td);
        
        if ((dayOfWeekNum === 0 && day !== daysInMonth) || day === daysInMonth) {
            while (row.children.length < 7) {
                const td = document.createElement("td");
                td.textContent = "";
                td.className = "empty-cell";
                row.appendChild(td);
            }
            tbody.appendChild(row);
            row = document.createElement("tr");
        }
    }
    
    table.appendChild(tbody);
    calendarContainer.appendChild(table);
}

function selectDate(year, month, day) {
    selectedDate = new Date(year, month, day);
    const selectedDateDisplay = document.getElementById("selected-date");
    if (selectedDateDisplay) {
        selectedDateDisplay.textContent = `Выбранная дата: ${day} ${months[month]} ${year} года`;
        selectedDateDisplay.style.opacity = "0";
        setTimeout(() => { selectedDateDisplay.style.opacity = "1"; }, 100);
    }
    renderCalendar();
}

function prevMonth() { currentDate.setMonth(currentDate.getMonth() - 1); renderCalendar(); }
function nextMonth() { currentDate.setMonth(currentDate.getMonth() + 1); renderCalendar(); }
function goToToday() { currentDate = new Date(); renderCalendar(); }

document.addEventListener("DOMContentLoaded", function() {
    renderCalendar();
});

// ==================== ЗАДАЧА 3: DOM-ДЕРЕВО (ТОЛЬКО СПИСКИ) ====================
document.addEventListener("DOMContentLoaded", function() {
    const countBtn = document.getElementById("count-btn");
    if (countBtn) {
        countBtn.addEventListener("click", function() {
            const ulLists = document.getElementsByTagName("ul");
            const olLists = document.getElementsByTagName("ol");
            const totalLists = ulLists.length + olLists.length;
            const allLists = [...ulLists, ...olLists];
            const result = document.getElementById("dom-result");
            if (result) {
                result.innerHTML = `
                    <strong>Найдено списков: ${totalLists}</strong><br>
                    - Маркированных списков (&lt;ul&gt;): ${ulLists.length}<br>
                    - Нумерованных списков (&lt;ol&gt;): ${olLists.length}
                `;
                highlightLists(allLists);
            }
        });
    }
});

function highlightLists(lists) {
    document.querySelectorAll('.highlight-list').forEach(list => list.classList.remove('highlight-list'));
    lists.forEach(list => {
        list.classList.add('highlight-list');
        setTimeout(() => list.classList.remove('highlight-list'), 2000);
    });
}

// ==================== ЗАДАЧА 4: ТЕКСТ В 5 БЛОКАХ (30 мс, ПОСЛЕДОВАТЕЛЬНО) ====================
(function() {
    const blocks = document.querySelectorAll('#task4-blocks .task4-block');
    let task4Interval = null;
    let currentBlockIdx = 0;

    function clearAllTexts() {
        blocks.forEach((block, i) => {
            block.textContent = `Блок ${i+1}`;
            block.classList.remove('active-text');
        });
    }

    function showTextInBlock(index) {
        clearAllTexts(); // очищаем все блоки
        if (blocks[index]) {
            blocks[index].textContent = `✨ Текст ${index+1} ✨\n${new Date().toLocaleTimeString()}`;
            blocks[index].classList.add('active-text');
        }
    }

    function startTask4() {
        if (task4Interval) clearInterval(task4Interval);
        task4Interval = setInterval(() => {
            showTextInBlock(currentBlockIdx);
            currentBlockIdx = (currentBlockIdx + 1) % blocks.length;
        }, 30);
    }

    function stopTask4() {
        if (task4Interval) {
            clearInterval(task4Interval);
            task4Interval = null;
        }
        clearAllTexts();
    }

    const startBtn = document.getElementById('task4-start-btn');
    const stopBtn = document.getElementById('task4-stop-btn');
    if (startBtn && stopBtn) {
        startBtn.addEventListener('click', startTask4);
        stopBtn.addEventListener('click', stopTask4);
        startTask4();
    }
})();

// ==================== ЗАДАЧА 5: ССЫЛКИ В DIV, УДАЛЕНИЕ СВЕРХУ ====================
(function() {
    const container = document.getElementById('dynamic-links');
    if (!container) return;

    function addLink() {
        let linkText = prompt('Введите текст ссылки:', 'Пример ссылки');
        if (linkText === null) return;
        
        const wrapper = document.createElement('div');
        wrapper.className = 'link-wrapper';
        
        const link = document.createElement('a');
        link.href = '#';
        link.textContent = linkText;
        link.style.color = '#1e88e5';
        link.style.textDecoration = 'none';
        link.onclick = (e) => {
            e.preventDefault();
            alert(`Переход по ссылке: ${linkText}`);
        };
        
        wrapper.appendChild(link);
        container.appendChild(wrapper);
    }

    function removeTopLink() {
        const firstItem = container.firstElementChild;
        if (!firstItem) {
            alert('Нет элементов для удаления');
            return;
        }
        const link = firstItem.querySelector('a');
        const content = link ? link.textContent : 'неизвестный элемент';
        if (confirm(`Удалить элемент: "${content}"?`)) {
            alert(`Удалено содержимое: ${content}`);
            firstItem.remove();
        }
    }

    document.getElementById('add-link-btn')?.addEventListener('click', addLink);
    document.getElementById('remove-top-btn')?.addEventListener('click', removeTopLink);
})();

// ==================== ЗАДАЧА 6: ВЫХОД МЫШИ С ЯЧЕЙКИ → КОПИРОВАТЬ ТЕКСТ ВНИЗ ====================
(function() {
    const cells = document.querySelectorAll('.task6-cell');
    const displayDiv = document.getElementById('task6-display');

    function copyCellText(event) {
        const cell = event.currentTarget;
        if (displayDiv) {
            displayDiv.textContent = cell.textContent;
        }
    }

    cells.forEach(cell => {
        cell.addEventListener('mouseleave', copyCellText);
    });
})();

// ==================== ЗАДАЧА 7: МЕНЮ СЛАДОСТЕЙ ====================
(function() {
    const sweetsList = document.getElementById('sweetsList');
    const sweetsMessage = document.getElementById('sweetsMessage');
    const resetSweetsBtn = document.getElementById('resetSweetsBtn');

    if (!sweetsList || !sweetsMessage || !resetSweetsBtn) return;

    function checkAllSweetsGone() {
        const visibleItems = sweetsList.querySelectorAll('li:not(.sweet-hidden)');
        sweetsMessage.style.display = visibleItems.length === 0 ? 'block' : 'none';
    }

    function hideSweet(li, callback) {
        li.classList.add('sweet-fade-out');

        setTimeout(function() {
            li.classList.add('sweet-hidden');
            
            if (typeof callback === 'function') {
                callback();
            }
        }, 300);
    }

    sweetsList.addEventListener('click', function(event) {
        const li = event.target.closest('li');
        if (!li || li.classList.contains('sweet-hidden')) return;

        hideSweet(li, checkAllSweetsGone);
    });

    resetSweetsBtn.addEventListener('click', function() {
        const allSweets = sweetsList.querySelectorAll('li');
        allSweets.forEach(function(li) {
            li.classList.remove('sweet-fade-out', 'sweet-hidden');
        });
        sweetsMessage.style.display = 'none';
    });
})();

// ==================== ЗАДАЧА 8: ПРОЗРАЧНОСТЬ КАРТИНКИ ПРИ НАВЕДЕНИИ ====================
(function() {
    const hoverImage = document.getElementById('hoverImage');
    if (!hoverImage) return;

    hoverImage.addEventListener('mouseover', function() {
        hoverImage.classList.add('fade-out');
    });

    hoverImage.addEventListener('mouseout', function() {
        hoverImage.classList.remove('fade-out');
    });
})();

// ==================== ЗАДАЧА 9 ====================

(function() {

    const form = document.getElementById("simpleForm");

    if (!form) return;

    form.addEventListener("submit", function(e) {

        e.preventDefault();

        validateSimple();

    });

    function validateSimple() {

        const name = document.getElementById("simpleName");
        const email = document.getElementById("simpleEmail");
        const password = document.getElementById("simplePassword");

        let valid = true;

        clearErrors();

        const nameRegex = /^[А-Яа-яA-Za-zЁё\s]{2,}$/;
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;

        if (!nameRegex.test(name.value)) {
            showError(name, "Введите минимум 2 буквы");
            valid = false;
        } else {
            showSuccess(name);
        }

        if (!emailRegex.test(email.value)) {
            showError(email, "Некорректный E-mail");
            valid = false;
        } else {
            showSuccess(email);
        }

        if (password.value.length < 6) {
            showError(password, "Минимум 6 символов");
            valid = false;
        } else {
            showSuccess(password);
        }

        return valid;
    }

})();

// ==================== ЗАДАЧА 10 ====================

(function() {

    const form = document.getElementById("fullForm");

    if (!form) return;

    form.addEventListener("submit", function(e) {

        e.preventDefault();

        validateFull();

    });

    function validateFull() {

        clearErrors();

        let valid = true;

        const fio = document.getElementById("fio");
        const login = document.getElementById("login");
        const email = document.getElementById("email");
        const phone = document.getElementById("phone");
        const date = document.getElementById("date");
        const faculty = document.getElementById("faculty");
        const department = document.getElementById("department");
        const password1 = document.getElementById("password1");
        const password2 = document.getElementById("password2");

        const fioRegex = /^[А-Яа-яЁёA-Za-z\s]{5,}$/;
        const loginRegex = /^[a-zA-Z0-9_]{4,16}$/;
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        const phoneRegex = /^\+?\d{11,12}$/;
        const dateRegex = /^\d{2}\.\d{2}\.\d{4}$/;

        if (!fioRegex.test(fio.value)) {
            showError(fio, "Минимум 5 букв");
            valid = false;
        } else showSuccess(fio);

        if (!loginRegex.test(login.value)) {
            showError(login, "4-16 символов");
            valid = false;
        } else showSuccess(login);

        if (!emailRegex.test(email.value)) {
            showError(email, "Некорректный email");
            valid = false;
        } else showSuccess(email);

        if (!phoneRegex.test(phone.value)) {
            showError(phone, "Введите 11 или 12 цифр");
            valid = false;
        } else showSuccess(phone);

        if (!dateRegex.test(date.value)) {
            showError(date, "Формат ДД.ММ.ГГГГ");
            valid = false;
        } else showSuccess(date);

        if (faculty.value.length < 2) {
            showError(faculty, "Минимум 2 символа");
            valid = false;
        } else showSuccess(faculty);

        if (department.value.length < 2) {
            showError(department, "Минимум 2 символа");
            valid = false;
        } else showSuccess(department);

        if (password1.value.length < 6) {
            showError(password1, "Минимум 6 символов");
            valid = false;
        } else showSuccess(password1);

        if (password1.value !== password2.value) {
            showError(password2, "Пароли не совпадают");
            valid = false;
        } else showSuccess(password2);

        return valid;
    }

})();

// ==================== ОБЩИЕ ФУНКЦИИ ====================

function showError(input, message) {

    input.classList.add("input-error");
    input.classList.remove("input-success");

    const error = input.parentElement.querySelector(".error-text");

    if (error) {
        error.textContent = message;
    }
}

function showSuccess(input) {

    input.classList.remove("input-error");
    input.classList.add("input-success");

    const error = input.parentElement.querySelector(".error-text");

    if (error) {
        error.textContent = "";
    }
}

function clearErrors() {

    document.querySelectorAll(".error-text").forEach(el => {
        el.textContent = "";
    });

    document.querySelectorAll("input").forEach(input => {
        input.classList.remove("input-error");
    });
}
// ==================== ЗАДАЧА 11: BLOCK BLAST (исправленная версия) ====================
(function() {
    const BOARD_SIZE = 8;
    let board = Array(BOARD_SIZE).fill().map(() => Array(BOARD_SIZE).fill(false));
    let currentShapes = [];     // массив из 3 объектов { matrix, type }
    let selectedShapeIndex = -1;
    let score = 0;
    let multiplier = 1;
    let gameActive = true;

    // ----- Точные фигуры из Тетриса + квадрат 3x3 -----
    const SHAPES_LIST = [
        { name: "I", matrix: [[1,1,1,1]] },                          // I (4x1)
        { name: "O", matrix: [[1,1],[1,1]] },                        // O (2x2)
        { name: "T", matrix: [[0,1,0],[1,1,1]] },                    // T
        { name: "S", matrix: [[0,1,1],[1,1,0]] },                    // S
        { name: "Z", matrix: [[1,1,0],[0,1,1]] },                    // Z
        { name: "J", matrix: [[1,0,0],[1,1,1]] },                    // J (поворот влево)
        { name: "L", matrix: [[0,0,1],[1,1,1]] },                    // L (поворот вправо)
        { name: "Square3x3", matrix: [[1,1,1],[1,1,1],[1,1,1]] }     // дополнительный квадрат 3x3
    ];

    // Получить случайную фигуру из списка (копия матрицы)
    function getRandomShape() {
        const idx = Math.floor(Math.random() * SHAPES_LIST.length);
        const original = SHAPES_LIST[idx];
        const matrixCopy = original.matrix.map(row => [...row]);
        return { matrix: matrixCopy, type: original.name };
    }

    // Генерация трёх новых фигур (вызывается только после размещения всех трёх)
    function generateNewShapesSet() {
        currentShapes = [];
        for (let i = 0; i < 3; i++) {
            currentShapes.push(getRandomShape());
        }
        selectedShapeIndex = -1;
        renderShapesPanel();
        updateSelectedInfo();
        removeHighlights();
        // После генерации новой тройки сразу проверяем, есть ли возможный ход
        if (!anyMovePossible()) {
            gameActive = false;
            endGame();
        }
    }

    // Проверка: можно ли поставить хотя бы одну из текущих фигур?
    function anyMovePossible() {
        for (let idx = 0; idx < currentShapes.length; idx++) {
            const matrix = currentShapes[idx].matrix;
            for (let i = 0; i <= BOARD_SIZE - matrix.length; i++) {
                for (let j = 0; j <= BOARD_SIZE - matrix[0].length; j++) {
                    if (canPlace(matrix, i, j)) {
                        return true;
                    }
                }
            }
        }
        return false;
    }

    // Отрисовка панели с фигурами (без кликабельных событий, они вешаются отдельно)
    function renderShapesPanel() {
        const container = document.getElementById('blast-shapes-container');
        if (!container) return;
        container.innerHTML = '';
        currentShapes.forEach((shape, idx) => {
            const matrix = shape.matrix;
            const rows = matrix.length;
            const cols = matrix[0].length;
            const card = document.createElement('div');
            card.className = 'shape-card';
            if (selectedShapeIndex === idx) card.classList.add('selected-shape');
            const grid = document.createElement('div');
            grid.style.display = 'grid';
            grid.style.gridTemplateColumns = `repeat(${cols}, 28px)`;
            grid.style.gap = '2px';
            grid.style.background = '#bdc3c7';
            grid.style.padding = '4px';
            grid.style.borderRadius = '8px';
            for (let r = 0; r < rows; r++) {
                for (let c = 0; c < cols; c++) {
                    const cell = document.createElement('div');
                    cell.className = 'shape-cell';
                    if (matrix[r][c] === 1) cell.classList.add('active-block');
                    else cell.style.backgroundColor = '#ecf0f1';
                    grid.appendChild(cell);
                }
            }
            card.appendChild(grid);
            // Клик по фигуре – выбор
            card.addEventListener('click', (e) => {
                e.stopPropagation();
                if (!gameActive) return;
                if (selectedShapeIndex === idx) {
                    selectedShapeIndex = -1;
                } else {
                    selectedShapeIndex = idx;
                }
                renderShapesPanel();
                updateSelectedInfo();
                removeHighlights();
            });
            container.appendChild(card);
        });
    }

    function updateSelectedInfo() {
        const infoDiv = document.getElementById('blast-selected-info');
        if (infoDiv) {
            if (selectedShapeIndex >= 0) {
                infoDiv.innerHTML = `✅ Выбран блок: ${currentShapes[selectedShapeIndex].type}`;
            } else {
                infoDiv.innerHTML = `❌ Выбран блок: нет (кликни на фигуру)`;
            }
        }
    }

    function renderBoard() {
        const boardTable = document.getElementById('blast-board');
        if (!boardTable) return;
        boardTable.innerHTML = '';
        for (let i = 0; i < BOARD_SIZE; i++) {
            const tr = document.createElement('tr');
            for (let j = 0; j < BOARD_SIZE; j++) {
                const td = document.createElement('td');
                if (board[i][j]) td.classList.add('filled');
                td.dataset.row = i;
                td.dataset.col = j;
                td.addEventListener('click', (function(r,c) {
                    return function() { onCellClick(r,c); };
                })(i,j));
                td.addEventListener('mouseenter', (function(r,c) {
                    return function() { onCellHover(r,c); };
                })(i,j));
                td.addEventListener('mouseleave', () => removeHighlights());
                tr.appendChild(td);
            }
            boardTable.appendChild(tr);
        }
    }

    function canPlace(matrix, startRow, startCol) {
        const rows = matrix.length;
        const cols = matrix[0].length;
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                if (matrix[r][c] === 1) {
                    const nr = startRow + r;
                    const nc = startCol + c;
                    if (nr >= BOARD_SIZE || nc >= BOARD_SIZE || nr < 0 || nc < 0) return false;
                    if (board[nr][nc]) return false;
                }
            }
        }
        return true;
    }

    function placeShape(matrix, startRow, startCol) {
        const rows = matrix.length;
        const cols = matrix[0].length;
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                if (matrix[r][c] === 1) {
                    board[startRow + r][startCol + c] = true;
                }
            }
        }
    }

    // Очистка заполненных линий и столбцов, возвращает количество очищенных линий
    function clearLinesAndColumns() {
        let clearedRows = [];
        let clearedCols = [];
        // строки
        for (let i = 0; i < BOARD_SIZE; i++) {
            let full = true;
            for (let j = 0; j < BOARD_SIZE; j++) {
                if (!board[i][j]) { full = false; break; }
            }
            if (full) clearedRows.push(i);
        }
        // столбцы
        for (let j = 0; j < BOARD_SIZE; j++) {
            let full = true;
            for (let i = 0; i < BOARD_SIZE; i++) {
                if (!board[i][j]) { full = false; break; }
            }
            if (full) clearedCols.push(j);
        }
        // удаляем строки
        for (let row of clearedRows) {
            for (let j = 0; j < BOARD_SIZE; j++) board[row][j] = false;
        }
        for (let col of clearedCols) {
            for (let i = 0; i < BOARD_SIZE; i++) board[i][col] = false;
        }
        return clearedRows.length + clearedCols.length;
    }

    function addScoreAndMultiplier(linesCleared) {
        if (linesCleared === 0) {
            multiplier = 1;
        } else {
            let lineBonus = linesCleared === 1 ? 10 : (linesCleared === 2 ? 30 : 60);
            let gained = lineBonus * multiplier;
            score += gained;
            multiplier += linesCleared;
            if (multiplier > 10) multiplier = 10;
        }
        updateUI();
    }

    function updateUI() {
        document.getElementById('blast-score').innerText = score;
        document.getElementById('blast-multiplier').innerText = multiplier;
        renderBoard();
    }

    function endGame() {
        gameActive = false;
        const msgDiv = document.getElementById('blast-message');
        if (msgDiv) {
            msgDiv.innerHTML = `💀 ИГРА ОКОНЧЕНА! Счёт: ${score} 💀<br>Нажмите "Новая игра"`;
            msgDiv.style.background = "#e74c3c";
            msgDiv.style.color = "white";
        }
    }

    // Обработка клика по ячейке поля
    function onCellClick(row, col) {
        if (!gameActive) {
            alert("Игра окончена. Начните новую.");
            return;
        }
        if (selectedShapeIndex === -1) {
            alert("Сначала выбери блок (кликни на фигуру справа)");
            return;
        }
        const shape = currentShapes[selectedShapeIndex];
        if (!shape) return;
        const matrix = shape.matrix;
        if (canPlace(matrix, row, col)) {
            // Размещаем фигуру
            placeShape(matrix, row, col);
            const linesCleared = clearLinesAndColumns();
            addScoreAndMultiplier(linesCleared);
            
            // Удаляем использованную фигуру из текущего набора
            currentShapes.splice(selectedShapeIndex, 1);
            selectedShapeIndex = -1;
            
            // Обновляем отображение панели и поля
            renderShapesPanel();
            renderBoard();
            updateSelectedInfo();
            removeHighlights();
            
            // Проверяем, если текущих фигур не осталось (все три использованы) -> генерируем новые 3
            if (currentShapes.length === 0) {
                generateNewShapesSet();
            } else {
                // Если фигуры ещё есть, проверяем, возможен ли ход
                if (!anyMovePossible()) {
                    gameActive = false;
                    endGame();
                }
            }
        } else {
            alert("Сюда нельзя поставить блок!");
        }
    }

    function onCellHover(row, col) {
        if (!gameActive) return;
        if (selectedShapeIndex === -1) return;
        const shape = currentShapes[selectedShapeIndex];
        if (!shape) return;
        const matrix = shape.matrix;
        removeHighlights();
        if (canPlace(matrix, row, col)) {
            highlightShape(matrix, row, col);
        }
    }

    function highlightShape(matrix, startRow, startCol) {
        const rows = matrix.length;
        const cols = matrix[0].length;
        const cells = document.querySelectorAll('#blast-board td');
        for (let r = 0; r < rows; r++) {
            for (let c = 0; c < cols; c++) {
                if (matrix[r][c] === 1) {
                    const nr = startRow + r;
                    const nc = startCol + c;
                    if (nr >= 0 && nr < BOARD_SIZE && nc >= 0 && nc < BOARD_SIZE) {
                        const idx = nr * BOARD_SIZE + nc;
                        if (cells[idx]) cells[idx].classList.add('hover-highlight');
                    }
                }
            }
        }
    }

    function removeHighlights() {
        const cells = document.querySelectorAll('#blast-board td');
        cells.forEach(cell => cell.classList.remove('hover-highlight'));
    }

    function newGame() {
        board = Array(BOARD_SIZE).fill().map(() => Array(BOARD_SIZE).fill(false));
        score = 0;
        multiplier = 1;
        gameActive = true;
        selectedShapeIndex = -1;
        // генерируем первый набор из 3 фигур
        currentShapes = [];
        for (let i = 0; i < 3; i++) {
            currentShapes.push(getRandomShape());
        }
        renderBoard();
        renderShapesPanel();
        updateUI();
        updateSelectedInfo();
        removeHighlights();
        const msgDiv = document.getElementById('blast-message');
        if (msgDiv) {
            msgDiv.innerHTML = '';
            msgDiv.style.background = "";
            msgDiv.style.color = "";
        }
        // Проверка возможного хода
        if (!anyMovePossible()) {
            gameActive = false;
            endGame();
        }
    }

    // Инициализация
    document.addEventListener('DOMContentLoaded', function() {
        const newGameBtn = document.getElementById('blast-newgame');
        if (newGameBtn) newGameBtn.addEventListener('click', newGame);
        newGame();
    });
})();